...
if(count(explode('/', $url)) > 2) {
	for($i = 0; $i < count(explode('/', $url));$i++) {
		if(array_key_exists(1,explode('/', $url))) {
			$languages =  select_one('languages',"title = '".explode('/', $url)[1]."' AND disabled = 0",__FILE__,__LINE__);

			if($languages) {
				@define('LANG',$languages['title']);
			} else {
				$languages =  select_one('languages',"default_for_site = 1",__FILE__,__LINE__);
				@define('LANG', '');
...
$pages =  select('pages','published = 1','',__FILE__,__LINE__ );
...
if(ALIAS_0 && LANG) {
	$page =  select_one('pages',"alias_".LANG." = '".ALIAS_0."'",__FILE__,__LINE__);
	$module =  select_one('modules',"alias_".LANG." = '".ALIAS_0."'",__FILE__,__LINE__);
...
if(!empty($_SESSION['login_id'])) {
	$user =  select_one('users', 'id = '.$_SESSION['login_id'],__FILE__,__LINE__);
	if($user) {
		@define('LI', $user['id']);
		@define('LN', $user['name']);
