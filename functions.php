...
function get_module_url($name) {
	$url = '';
	$modules = select_one('modules', "name = '".$name."'", __FILE__,__LINE__);
...
function get_default_module_url($name) {
	$url = '';
	$modules = select_one('modules', "default_module = 1", __FILE__,__LINE__);
...
function control_languages_columns() {
	if(get_langs()) {
		...
		$columns = [];
		$modules_and_pages = select('modules,pages', "db_table != ''", '', __FILE__,__LINE__);
		...
		foreach(get_langs() as $lang) {
			foreach($tables as $table) {
				...
				$columns = select('modules_columns', "languages = 1 AND db_table = '".$checked_table."'", '', __FILE__,__LINE__);
		...
		$modules_columns = select('modules_columns', "languages = 1 AND ".$columns_select_condition, '', __FILE__,__LINE__);
...
function get_langs($condition = 'disabled = 0') {
	$languages = select('languages',$condition,'',__FILE__,__LINE__);
...
function get_to_default_url() {
	$module = select_one('modules',"default_module = 1", __FILE__,__LINE__ );
								  
	$lang = select_one('languages',"default_for_site = 1",__FILE__,__LINE__ );				 
...
function enable_def_url() {
	...
	if(LANG) {
		$lang = select_one('languages',"disabled = 0 AND title = '".LANG."'",__FILE__,__LINE__ );
	...
	if(ALIAS_0 && LANG) {
		$pages = select_one('pages',"alias_".LANG." = '".ALIAS_0."'",__FILE__,__LINE__ );
		
		$modules = select_one('modules',"alias_".LANG." = '".ALIAS_0."'",__FILE__,__LINE__ );
